...
class Executor {
  ...
  async poll () {
    while (true) {
      ...
      if (!group) return // this._schedule.detectStuck()
  ...
  async _request (group) {
    group.started()
    // console.log('request', group.getShard(), [...group.values()].length)
  ...
  async _writeShard (group) {
    try {
      ...
      // console.log('SUCCESS', group.getShard(), [...group.values()].length)
      this._groupCompleted(group)
    } catch (error) {
      // console.log('FAILURE', error.code, group.getShard(), [...group.values()].length)
