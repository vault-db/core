...
class Schedule {
  ...
  _findGroup (op, depGroups) {
    ...
    groups = groups.map((id) => this._groups.get(id))
    ...
    while (index + 1 < groups.length && depth > groups[index].depth) {
      index += 1
    }
  ...
  _placeOpInGroup (op, group, depGroups) {
    ...
    let descendants = [group.id, ...group.descendants].map((id) => {
      return this._groups.get(id)
    })

    let downstreamShards = new Set(descendants.map((group) => group.shard))

    for (let shard of downstreamShards) {
      this._sortShard(shard)
    }
    this._updateDepth(descendants)
  }
  ...
  _updateDepth (descendants) {
    this._sortTopological(descendants)

    for (let desc of descendants) {
      let parents = [...desc.parents].map((id) => this._groups.get(id))
      desc.depth = parents.reduce((d, g) => Math.max(d, g.depth + 1), 0)
    }
  }

  _sortShard (shard) {
    let shardGroups = this._shards.get(shard).map((id) => {
      return this._groups.get(id)
    })

    this._sortTopological(shardGroups)

    for (let [idx, group] of shardGroups.entries()) {
      group.index = idx
    }
    this._shards.set(shard, shardGroups.map((group) => group.id))
  }
...
class LowerBound {
  ...
  get () {
    let groupAncestors = this._depGroups.flatMap((group) => {
      return [...group.ancestors].map((id) => this._schedule._groups.get(id))
    })
