...
class Schedule {
  ...
  _findGroup (op) {
    ...
    let minIndex = this._findIndex(0, op.shard, op.deps, 0)
    minIndex = this._findIndex(minIndex, op.shard, indirectDeps, 1)
  ...
  _findIndex (minIndex, shard, deps, offset) {
    for (let depId of deps) {
      let dep = this._operations.get(depId)

      if (dep.shard === shard) {
        minIndex = Math.max(minIndex, dep.index + offset)
      }
    }
    return minIndex
  }
