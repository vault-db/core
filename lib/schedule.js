...
class Schedule {
  ...
  _findGroup (op, depGroups) {
    ...
    let { type, idx } = new LowerBound(this, op, groups, depGroups).get()
...
class LowerBound {
  constructor (schedule, op, groups, depGroups) {
  ...
  get () {
    let groupAncestors = this._depGroups.flatMap((group) => [...group.ancestors])

    let depIds = new Set(this._depGroups.map((group) => group.id))
    let ancIds = new Set(groupAncestors)
    let idx = this._groups.length

    while (idx--) {
      let { id, state } = this._groups[idx]
