...
class Task {
  ...
  async update (pathStr, fn) {
    ...
    // TODO: what happens if fn() throws? should we move it earlier, i.e. after
    // all relevant shards have been read, but before the link() calls are
    // planned
    //
    // ... probably not, it could lead to lost updates when multiple writes to
    // the same doc are bundled together -- each update() needs to see the
    // state produced by others in the same batch, not the state from when the
    // shard was first read
    //
    // ... I just thought we could avoid doing the link() calls if fn() is
    // going to fail
  ...
  // TODO: figure out how to trigger the conflict code path
  //
  // answer: it requires a *sync* call to poll() below, b/c that prevents an
  // update() from the same tick being merged into the same group, and it
  // requires ignoring conflicts here. fixing Schedule to prevent concurrent
  // requests for the same shard prevents conflicts happening at all

  async _retryOnConflict (planner, n = 1) {
    let ops = await planner()

    // TODO: do this async so other work can make it into the next group
