...
class Shard {
  static parse (string, cipher) {
    if (!string) {
      let index = new Cell(cipher, JsonCodec).set([])
      return new Shard(cipher, index, [])
    }

    let [header, index, ...items] = string.split('\n')
    index = new Cell(cipher, JsonCodec, index)
    items = items.map((item) => new Cell(cipher, JsonCodec, item))
  ...
  async _getOr (path, init) {
    ...
    if (idx < 0) {
      ...
      let item = new Cell(this._cipher, JsonCodec).set(init)
